// TODO:
// check https://github.com/tensorflow/tfjs-examples/tree/master/mnist-acgan && https://machinelearningmastery.com/how-to-develop-an-auxiliary-classifier-gan-ac-gan-from-scratch-with-keras/
// and https://github.com/tensorflow/tfjs-examples/tree/master/fashion-mnist-vae && https://blog.keras.io/building-autoencoders-in-keras.html
import * as tf from "@tensorflow/tfjs";

export const RANDOM_SEED = 123;

// Input params
export const BATCH = 100;
export const SEED_SIZE = 7;
// const SIZE = 28;
const INPUT_SIZE = SEED_SIZE;
const SEED_STD = 3.5;

const ONES = tf.ones([ BATCH, 1 ]);
const ONES_PRIME = tf.ones([ BATCH, 1 ]).mul(tf.scalar(.98));
const ZEROS = tf.zeros([ BATCH, 1 ]);

// Generator and discrimantor params
// const DISCRIMINATOR_LEARNING_RATE = .025;
// const GENERATOR_LEARNING_RATE = .025;
const DISCRIMINATOR_LEARNING_RATE = .5;
const GENERATOR_LEARNING_RATE = .5;
const dOptimizer = tf.train.sgd(DISCRIMINATOR_LEARNING_RATE);
const gOptimizer = tf.train.sgd(GENERATOR_LEARNING_RATE);

// Helper functions
// export const varInitNormal = (shape, mean=0, std=.1) => tf.variable(tf.randomUniform(shape));
export const varInitNormal = (shape, mean=0, std=.1) => tf.variable(tf.randomNormal(shape, mean, std));
// export const varLoad = (shape, data) => tf.variable(tf.tensor(shape, data));

// export const seed = (shape) => tf.randomNormal(shape, 0, SEED_STD, "int32", RANDOM_SEED);
export const seed = (shape) => tf.randomUniform(shape, 0, 2, "int32", RANDOM_SEED);


// Network arch for generator
// let G1w = varInitNormal([ SEED_SIZE, 7 ]);
// let G1b = varInitNormal([ 7 ]);
...
export default async function trainBatch(realBatch, fakeBatch) {
...
export const guess = async() => {
  ...
  return gen(seed([ 1, SEED_SIZE ])).data();
};
